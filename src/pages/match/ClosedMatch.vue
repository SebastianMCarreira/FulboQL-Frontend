<template>
  <q-page class="flex">
    ...
    <div v-bind:class="{ hidden: !matchLoaded }" class="full-width q-gutter-sm" style="padding: 15px;">
      <div class="row text-h5 sub-header justify-between">
        <div>
          {{ match.teamA.club.name }}
        </div>
        <div>{{ match.result.teamA }}</div>
        <div>VS.</div>
        <div>{{ match.result.teamB }}</div>
        <div>
      ...
      <q-list>
        ...
        <q-expansion-item
          ...
          label="Match events"
          @click="loadEvents()">
          <div class="flex q-px-xl" v-bind:class="{ 'justify-center': !eventsLoaded }">
            <q-circular-progress
                  indeterminate
                  color="primary"
                  class="q-ma-md "
                ...
                />
            <div v-bind:class="{ hidden: !eventsLoaded }"  class="full-width">
              <q-list>
                <q-item v-for="(event, index) in events" v-bind:key="index">
                  <q-item-section><event-row :eventData="event"/></q-item-section>
                </q-item>
              </q-list>
            </div>
          </div>
